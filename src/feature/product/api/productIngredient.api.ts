...
const productIngredientItemResponseSchema = apiItemResponseSchema(responseProductIngredientSchema)
...
export async function getProductIngredientByIdAPI(id: number) {
    try {
        const { data } = await api.get(`/product-ingredients/${id}`)
        return productIngredientItemResponseSchema.parse(data)
    } catch (error) {
        handleApiError(error)
    }
}
